<script lang="ts">
	...
	import { getWFValidarTareaEstado,  updateWFValidarTarea } from '$lib/api/admin/administracion/financial/solicitud/validar.remote';
			...
			{...updateWFValidarTarea.enhance(async ({ submit }) => {
				try {
					...
					if (updateWFValidarTarea.result?.success) {
						...
						goto('/admin/administracion/financial/dashboard');
					}
				} catch (e) {
