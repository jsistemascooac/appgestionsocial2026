<script lang="ts">
	...
	import { getWFValidarTareaEstado, getWFValidarTareaParametro, updateWFValidarTarea } from '$lib/api/admin/comite/sepe/solicitud/validar.remote';
			...
			{...updateWFValidarTarea.enhance(async ({ submit }) => {
				try {
					...
					if (updateWFValidarTarea.result?.success) {
						...
						goto('/admin/comite/sepe/dashboard');
					}
				} catch (e) {
