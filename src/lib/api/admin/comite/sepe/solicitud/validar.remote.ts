...
export const addWFValidarTarea = command(v.string(), async (id) => {
	...
	let comite = 'j20f41fb2d85l85'// locals.user.perfil.gs_entidad;
...
export const getWFValidarTarea = query(async () => {
	...
	try {
		const wf_validar_tarea = await locals.pb
			...
			.getOne(`${params.id}`, {
				expand:'wf_tarea.wf_workflow'
			});

		  //  console.log("getWFMovimiento ProcesoxxxZZ:", wf_validar_tarea)
...
export const updateWFValidarTarea = form(
	v.object({ estado: v.pipe(v.string(), v.nonEmpty('Por favor registre un estado')),
		valor: v.pipe(
				...
				v.minValue(0, 'EL valor debe ser igual o mayor 0')),
		observacion: v.string() }),
	async ({  estado,valor,observacion }) => {
		const { locals,params } = getRequestEvent();
		...
		try {
			let validar_tarea = await locals.pb
				...
				.update(`${validar_tarea.wf_movimiento}`, {wf_estado:estado, observacion,valor});	
			//	if (id !== undefined) {
			//await getWFValidarTareaMovimiento().refresh();
			//	}
			return { success: true };
