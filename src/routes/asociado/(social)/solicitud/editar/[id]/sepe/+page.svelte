<script lang="ts">
	...
	import Fallecimiento from '$lib/components/asociado/solicitud/defuncion.svelte';
	...
	import Def from '$lib/components/asociado/solicitud/defuncion.svelte';
...
/* 	let fallecimiento = $derived(
		wfMCa.fallecimiento ? new Date(wfMCa.fallecimiento).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]
	);
